"""
Based on: https://github.com/Microsoft/azure-pipelines-tasks/issues/3027#issuecomment-391298035
...
def operate_recursively(path):
    for file in os.listdir(path):
        new_path = "{}/{}".format(path, file)
        if file.endswith(".html"):
            print("Embedding CSS in", new_path)
            embed_css_in_html_file(os.path.join(path, file), path)
        elif os.path.isdir(new_path):
            operate_recursively(new_path)


if __name__ == "__main__":
    operate_recursively(COVERAGE_REPORT_DIR)
